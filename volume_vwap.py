...
def detect_vwap_reversion(price: float, vwap_1h: float, vwap_1d: float,
    ...
    if deviation < -VWAP_REVERSION_THRESHOLD:
        ...
        # Цена выше VWAP
        # Если отклонение умеренное (<3%) и объём растёт — это бычий тренд, а не перекупленность
        if deviation < 0.03 and (volume_trend == 'rising' or is_spike):
            # Бычий тренд: цена выше VWAP с растущим объёмом
            strength = min(1.0, abs_deviation * 20 + 0.3)  # 1%→0.5, 2%→0.7
            return 'trend_bullish', strength
        
        # Сильное отклонение (>3%) — реальная перекупленность
...
def evaluate(symbol: str, current_price: float,
    ...
    elif signal == 'trend_bullish':
        score = int(55 + strength * 35)  # 55-90 — бычий тренд: цена растёт с объёмом
