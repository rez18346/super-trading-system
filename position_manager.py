...
class PositionManager:
    ...
    def open_position(self, symbol: str, direction: Direction,
        ...
        # 🛡️ Записываем сделку в историю (и в дашборд)
        try:
            db.add_trade(
                symbol=symbol,
                ...
                order_id=entry_order_id or '',
                exchange_id=entry_order_id or '',
                ts=datetime.now(timezone.utc).isoformat(),
            )
        except Exception as e_trade:
            logger.debug(f"[PM] add_trade error: {e_trade}")
    ...
    def close_position(self, symbol: str, exit_price: float,
        ...
        # Расчёт PnL
        # 🩹 Всегда пересчитываем по exit_price, а не используем кэшированный pos.pnl_usd
        # (pos.pnl_usd считается по mark-price, а не по реальной цене продажи)
        ...
        # 💰 Возвращаем капитал (с пересчитанным PnL!)
        cost = pos.entry_cost
        ...
        # 🩹 Записываем в PG trade_history (exit) — ТОЛЬКО если exit_price != entry_price
        # (предотвращает фейковые записи при неисполненных ордерах)
        _real_exit = abs(exit_price - pos.entry_price) / max(pos.entry_price, 0.0001) if pos.entry_price > 0 else 1.0
        if _real_exit > 0.001:  # exit_price хотя бы на 0.1% отличается от entry — считаем реальным
            try:
                db.add_trade(
                    symbol=symbol,
            ...
            logger.debug(f"⏭️ [PM] {symbol}: пропускаю запись exit (exit_price={exit_price:.4f} ≈ entry_price={pos.entry_price:.4f})")
