...
def _set_cvd_ws_ref(ws):
    """Сохранить ссылку на WS-клиент для CVD (вызывается после инициализации)."""
    global _ws_client_for_cvd
    _ws_client_for_cvd = ws
...
def _get_symbol_cvd(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        logger.warning(f"CVD error for {symbol}: {e}")
    return None
...
class IndustrialTrader:
    ...
    # 🛡️ Ручные позиции — система их не трогает
    HAND_SYMBOLS = {'BTC/USDT', 'ADA/USDT'}
    ...
    def __init__(self, config_path: str):
        ...
        self.last_exit_prices: Dict[str, Dict] = {}

        # 🌐 WebSocket для CVD (запускаем асинхронно в фоне)
        self._cvd_ws_started = False
        try:
            from ws_client import BybitWebSocketClient, set_global_client
            _raw_symbols = ['BTC', 'ETH', 'SOL', 'XRP', 'ADA', 'AVAX', 'DOT', 'DOGE', 'LINK',
                            'ATOM', 'ALGO', 'EGLD', 'APT', 'ARB', 'ROSE', 'MNT', 'FIL', 'NEAR', 'OP']
            _ws_symbols = [s + 'USDT' for s in _raw_symbols]
            ws = BybitWebSocketClient(symbols=_ws_symbols)
            set_global_client(ws)
            _set_cvd_ws_ref(ws)
            ws.start()
            self._cvd_ws_started = True
            logger.info(f"✅ CVD WebSocket запущен для {len(_ws_symbols)} символов")
        except Exception as e:
            logger.warning(f"⚠️ CVD WebSocket не запущен: {e}")
                ...
                if currency != 'USDT' and amount > 0.000001:  # Минимальный порог
                    try:
                        ...
                        if value_usdt > 1.0:  # Позиции больше $1
                            ...
                            # 🛡️ Пропускаем ручные позиции
                            if hasattr(self.__class__, 'HAND_SYMBOLS') and symbol in self.__class__.HAND_SYMBOLS:
                                logger.info(f"   🟢 {symbol}: ручная позиция (HAND_SYMBOLS) — не трогаем")
                                continue
    ...
    def get_average_buy_price(self, symbol: str, current_amount: float):
        """Получение цены входа — последнего BUY ордера Бинанса, не DCA-усреднение"""
        try:
            # 🩹 Берём ТОЛЬКО последний закрытый BUY ордер, а не среднюю за 48ч
            # (DCA-усреднение искажало entry_price при рестарте, что ломало PnL)
            ...
            # Берём последний BUY ордер по времени
            buy_orders.sort(key=lambda x: x['timestamp'], reverse=True)
            last_buy = buy_orders[0]

            price = last_buy.get('average', last_buy.get('price', 0))
            if not price or price == 0:
                return None, None

            entry_time = datetime.fromtimestamp(last_buy['timestamp'] / 1000).isoformat()
            return float(price), entry_time

        except Exception as e:
            logger.warning(f"Не удалось получить цену входа для {symbol}: {e}")
            return None, None
    ...
    def calculate_position_size(self, symbol: str, price: float, score: float = 65.0) -> float:
        """Расчет размера позиции: весь капитал делится на 3 монеты.
           Первый вход 30% от доли (пирамида), доливки догоняют до 100%. """
        risk_config = self.config['risk_management']

        # Максимум открытых позиций (по умолчанию 3)
        max_positions = risk_config.get('max_open_positions', 3)
        # Доля капитала на одну монету
        share_per_coin = self.available_capital / max_positions

        # Размер от Score: чем выше уверенность, тем больше вход
        if score >= 80:
            first_entry_pct = 50.0  # 50% от доли
            tier = f"Score>=80"
        elif score >= 75:
            first_entry_pct = 40.0
            tier = f"Score>=75"
        elif score >= 70:
            first_entry_pct = 35.0
            tier = f"Score>=70"
        else:
            first_entry_pct = 30.0
            tier = f"Score<70"

        # Первый вход — % от доли капитала на монету
        position_size = share_per_coin * (first_entry_pct / 100)

        # Ограничение от конфига (верхняя граница)
        max_order_limit = risk_config.get('max_buy_order_usd', 60.0)
        position_size = min(position_size, max_order_limit)

        # Минимальный размер
        min_position_usd = risk_config.get('min_position_usd', 5.0)
        position_size = max(position_size, min_position_usd)

        quantity = position_size / price
        logger.info(f"Размер позиции для {symbol}: ${position_size:.2f} ({tier}, score={score:.0f}, "
                     f"capital={self.available_capital:.2f}÷{max_positions}={share_per_coin:.2f}×{first_entry_pct:.0f}%), "
                     f"{quantity:.6f} units")
        return quantity
        ...
        try:
            ...
            if not self.config['bybit']['paper_trading']:
                ...
                if side == 'buy':
                    ...
                    # 🩹 Учитываем комиссию покупки в entry_price
                    # (комса продажи вычитается при sell — в PnL)
                    actual_entry = order_price
                    fee_data = order.get('fee', {})
                    if fee_data:
                        buy_fee_cost = float(fee_data.get('cost', 0))
                        buy_fee_cur = fee_data.get('currency', '')
                        if buy_fee_cost > 0 and buy_fee_cur == symbol.split('/')[0]:
                            # Комса в base монете — entry чуть выше
                            actual_fee_val = buy_fee_cost * order_price / actual_qty
                            actual_entry = order_price + actual_fee_val
                            logger.debug(f"   🩹 Комса покупки {symbol}: {buy_fee_cost:.6f} {buy_fee_cur} → entry коррекция +{actual_fee_val:.6f} (${actual_entry:.6f})")

                    # Добавляем новую позицию
                    # Используем order_price (цена BUY ордера), а не WA из БД
                    # Баг: DB.calculate_weighted_entry хранит старые цены, искажая PnL
                    with self._lock:
                        real_entry = actual_entry
                    ...
                    if symbol in self.positions:
                        ...
                        # 🩹 Вычитаем комиссию сделки из PnL
                        # (комиссия покупки уже учтена в entry_price — см. buy request ниже)
                        fee_data = order.get('fee', {})
                        trade_fee = float(fee_data.get('cost', 0)) if fee_data else 0.0
                        fee_asset = fee_data.get('currency', '') if fee_data else ''

                        pnl = (filled_price - entry) * filled - trade_fee
                        pnl_pct = ((filled_price - entry) / entry) * 100 if entry > 0 else 0
                        # На точное pnl_pct влияет trade_fee — для точности отнимаем процент
                        # (fee_asset может быть не USDT, но на практике в основном USDT/BASE)
                        ...
                        db.add_trade(
                            ...
                            order_id=str(order['id']),
                            exchange_id=str(order['id']),
                            fee_asset=fee_asset if fee_asset else None,
                            fee=float(trade_fee) if trade_fee > 0 else 0.0,
                            ts=datetime.now().isoformat()
                        )
                    ...
                    if symbol in self.positions:
                        ...
                        # 💰 Возвращаем капитал (сколько потратили на вход + PnL с учётом комсы)
                        filled = order.get('filled', quantity) or quantity
                        filled_price = order.get('average', order.get('price', price)) or price
                        fee_data = order.get('fee', {})
                        trade_fee = float(fee_data.get('cost', 0)) if fee_data else 0.0
                        profit = (filled_price - entry) * filled - trade_fee
    ...
    def trading_cycle(self):
        ...
        while self.running:
            try:
                ...
                # 🧹 Очистка старых лимитных ордеров (отключено по просьбе пользователя — ручные лимитники не снимать)
                ...
                # 🛡️ Обновляем счётчик сессионных входов под реальные позиции (каждый цикл)
                # Важно: _session_entries используется в TOP-3 для расчёта свободных слотов.
                # Без этого обновления трейдер застревает на старом значении (после init)
                # и не входит в новые сделки даже при свободных местах.
                self._session_entries = len(self.positions)
                ...
                try:
                    ...
                    if fetcher:
                        ...
                        for sym in symbols:
                            raw_5m = fetcher.get_ohlcv(sym, '5m', 50)
                ...
                # Если данных для монеты нет — используем глобальный BTC 1H тренд как прокси
                self._trend_1h = {}
                
                # Сначала вычисляем BTC 1H тренд (глобальный бенчмарк)
                _btc_trend_1h = 'neutral'
                if raw_btc_1h and len(raw_btc_1h) >= 20:
                    btc_closes = [c[4] for c in raw_btc_1h[-20:]]
                    btc_ema7 = sum(btc_closes[-7:]) / 7
                    btc_ema20 = sum(btc_closes[-20:]) / 20
                    btc_change = (btc_closes[-1] - btc_closes[0]) / btc_closes[0] * 100
                    if btc_ema7 > btc_ema20 and btc_change > 2:
                        _btc_trend_1h = 'strong_bullish'
                    elif btc_ema7 > btc_ema20:
                        _btc_trend_1h = 'bullish'
                    elif btc_ema7 < btc_ema20 and btc_change < -2:
                        _btc_trend_1h = 'strong_bearish'
                    elif btc_ema7 < btc_ema20:
                        _btc_trend_1h = 'bearish'
                ...
                for sym, h1_list in mtf_candles_1h.items():
                    if len(h1_list) >= 20:
                        ...
                        # Данных нет — используем BTC тренд как прокси
                        self._trend_1h[sym] = _btc_trend_1h
                ...
                if btc_5m_candles is not None and btc_1h_candles is not None:
                    try:
                        btc_current_price = btc_1h_candles.iloc[-1]['close'] if btc_1h_candles is not None and len(btc_1h_candles) > 0 else 0
                        regime_result = self._btc_regime.update(btc_5m_candles, btc_1h_candles, current_price=btc_current_price)
                        _htf = regime_result.get('htf_trend', '?')
                        _atr = regime_result.get('atr_1h', 0)
                        _vblock = regime_result.get('volatility_block', False)
                        _rblock = regime_result.get('range_block', False)
                        logger.info(f"🧠 BTC Regime: {regime_result['regime']} HTF={_htf} ATR={_atr} volBlock={_vblock} rangeBlock={_rblock} → rec={regime_result['recommendation']} ({regime_result['message']})")
                        ...
                        try:
                            ...
                            btc_analysis = {
                                ...
                                'atr_1h': regime_result.get('atr_1h', 0),
                                'volatility_block': regime_result.get('volatility_block', False),
                        ...
                        if not self._btc_regime.is_buy_allowed():
                            ...
                            self._btc_blocked = True  # флаг для основной логики, чтобы не входить
                        else:
                ...
                try:
                    ...
                    for _p in _recent_pnls:
                        if _p < 0:
                            ...
                            break
                    if True:
                        logger.info(f"📊 Портфель: PnL=\${_daily_pnl:.2f}, сделок={_daily_trades}, "
                                     f"профитных={_daily_wins}, открыто={_open_pos}, "
                                     f"экспозиция={_exposure:.0f}%")
                except Exception as _e:
                    logger.info(f"⚠️ [portfolio_stats] ошибка: {_e}")
                ...
                for symbol in symbols:
                    ...
                    if symbol not in self.positions:
                        try:
                            ...
                            de_rsi = analysis['rsi']
                            # 🛡️ de_positions = уникальные символы из self.positions + pm._positions
                            # Важно: не складывать — могут быть дубли (одна и та же позиция в двух хранилищах)
                            _pm_symbols = self._pm.position_symbols if hasattr(self, "_pm") else set()
                            de_positions = len(set(self.positions.keys()) | _pm_symbols)
                            ...
                            logger.info(f"🔍 [DE_PRE] {symbol}: de_positions={de_positions}, max_pos={max_pos}, DE будет вызван с лимитом {max_pos}, current={de_positions} > {max_pos} = {de_positions > max_pos}")
                            _c5m_raw = df.to_dict('records') if hasattr(df, 'to_dict') else None
                            # Конвертируем open/high/low/close/volume → o/h/l/c/v (как ожидают модули DE)
                            c5m = [{'o':r['open'],'h':r['high'],'l':r['low'],'c':r['close'],'v':r['volume']} for r in _c5m_raw] if _c5m_raw else None
                            ...
                            last_exit_info = self.last_exit_prices.get(symbol, {})
                            ...
                            try:
                                ...
                                if _ticker:
                                    high_24h = _ticker.get('high') or high_24h
                                    low_24h = _ticker.get('low') or low_24h
                            except Exception:
                                pass
                            # 🆕 ДВА НЕЗАВИСИМЫХ ПРОХОДА: LONG и SHORT — честное голосование
                            _long_result = de.decide_entry(
                                symbol, de_confidence, de_trend, de_rsi,
                            ...
                            # LONG score — base_score (без бонусов) для прохода порога
                            _long_score_raw = getattr(_long_result, 'score', 0) or 0
                            if _long_score_raw == 0 and hasattr(_long_result, 'metadata') and _long_result.metadata:
                                _long_score_raw = _long_result.metadata.get('base_score', _long_result.metadata.get('final_score', 0))

                            # SHORT score — base_score (без бонусов) для прохода порога
                            _short_score_raw = getattr(_short_result, 'score', 0) or 0
                            if _short_score_raw == 0 and hasattr(_short_result, 'metadata') and _short_result.metadata:
                                _short_score_raw = _short_result.metadata.get('base_score', _short_result.metadata.get('final_score', 0))

                            # VETO от BTC regime
                            _btc_regime_local = getattr(self._btc_regime, 'regime', 'unknown')
                            _long_veto = _btc_regime_local in ("forced_sell", "dump")
                            _short_veto = _btc_regime_local in ("recovery", "pump", "hype", "accumulation")

                            # Decision — честное сравнение двух независимых score
                            # Размер первого входа (30% от доли, пирамида дольёт остальное)
                            if _long_score_raw >= 80:
                                _size_mult = 0.5  # 50% при высоком скоре
                            elif _long_score_raw >= 70:
                                _size_mult = 0.4  # 40%
                            else:
                                _size_mult = 0.3  # 30% (база)

                            if _long_score_raw >= 55 and not _long_veto and (_short_score_raw < 55 or _short_veto or _long_score_raw >= _short_score_raw):
                                direction = "long"
                                decision_action = "enter"
                                _score = _long_score_raw
                                _de_result = _long_result
                                _reason = "LONG({:.0f}) > SHORT({:.0f}) veto={}".format(_long_score_raw, _short_score_raw, 'L' if _long_veto else 'S' if _short_veto else '-')
                            elif _short_score_raw >= 55 and not _short_veto:
                                direction = "short"
                                decision_action = "enter"
                                _score = _short_score_raw
                                _size_mult = _size_mult  # тот же размер
                                _de_result = _short_result
                                _de_result.side = "short"
                                _reason = "SHORT({:.0f}) > LONG({:.0f}) veto={}".format(_short_score_raw, _long_score_raw, 'L' if _long_veto else 'S' if _short_veto else '-')
                            else:
                                direction = "neutral"
                                ...
                                _size_mult = 0
                                _de_result = _long_result
                                _de_result.side = "long"
                                _low = _long_score_raw if not _long_veto else 0
                                _sh = _short_score_raw if not _short_veto else 0
                                _reason = "NEUTRAL L={:.0f} S={:.0f} need>=55".format(_low, _sh)

                            # Логируем модули (для vote_parser) — используем результат LONG (единый формат)
                            _de_votes = {}
                            if hasattr(_de_result, 'metadata') and _de_result.metadata:
                                _de_votes = _de_result.metadata.get('votes', {})
                            _vsa_o = _de_votes.get("vsa", {}).get("score", 50)
                            _vsa_d = _de_votes.get("vsa", {}).get("detail", "-")[:20]
                            _cvd_o = _de_votes.get("cvd", {}).get("score", 50)
                            _cvd_d = _de_votes.get("cvd", {}).get("detail", "N/A")[:30]
                            _liq_o = _de_votes.get("liquidity", {}).get("score", 50)
                            _liq_d = _de_votes.get("liquidity", {}).get("detail", "-")[:20]
                            _vv_o = _de_votes.get("volume_vwap", {}).get("score", 50)
                            _vv_d = _de_votes.get("volume_vwap", {}).get("detail", "-")[:20]
                            _mtf_o = _de_votes.get("mtf", {}).get("score", 50)
                            _adv_o = _de_votes.get("advisor", {}).get("score", 50)
                            _mlpro_o = _de_votes.get("ml_v2", {}).get("score", 50)
                            _rp_o = _de_votes.get("rp", {}).get("delta", 0)
                            _de_log = f"DE→{decision_action.upper()} {symbol}: score={_score} L={_long_score_raw} S={_short_score_raw}"
                            _de_log += f" ML-Pro:{_mlpro_o}(raw) Adv:{_adv_o}(raw) MTF:{_mtf_o}"
                            _de_log += f" RVB:0 Liq:{_liq_o}({_liq_d}) VV:{_vv_o}({_vv_d})"
                            _de_log += f" VSA:{_vsa_o}({_vsa_d}) CVD:{_cvd_o}({_cvd_d})"
                            _de_log += f" RP:{_rp_o}(N/A)"
                            # Добавляем бонусы в лог для диагностики
                            _bonus_data = _de_votes.get("bonus", {})
                            _price_bonus = _bonus_data.get("price", 0)
                            _rev_bonus = _bonus_data.get("reversal", 0)
                            _btc_bonus = _bonus_data.get("btc", 0)
                            if _price_bonus or _rev_bonus or _btc_bonus:
                                _de_log += f" bonus=p:{_price_bonus:.0f}+r:{_rev_bonus:.0f}+b:{_btc_bonus:.0f}"
                            logger.info(_de_log)
                            ...
                            if _score >= 65:
                                ...
                                logger.info("⭐ [{}→CANDIDATE] {}: direction={} score={:.0f}/100 | ${:.4f} | {}".format(
                                    direction.upper(), symbol, direction, _score, de_price, _reason))
                            else:
                                logger.info("⏳ [{}→HOLD] {}: {}".format(
                                    'HOLD' if direction == 'neutral' else direction.upper(), symbol, _reason))

                        except Exception as e_de:                            logger.warning(f"[DE] {symbol}: ошибка: {e_de}")
                    ...
                    # ════════════════════════════════════════════════════════════
                        ...
                        # 🛡️ Ручные позиции — не трогаем
                        if symbol in self.HAND_SYMBOLS:
                            logger.debug(f"   🟢 {symbol}: ручная позиция — stale пропущен")
                            continue
                        ...
                        # # 🎯 РАННИЙ ТРЕЙЛИНГ: ОТКЛЮЧЁН по решению пользователя (2026-07-11)
                        # # Срабатывал при PnL>=1.5% и откате 0.3%, но не давал цене дойти до 2%.
                        ...
                        # ═══════════════════════════════════════════════════════
                        # СТРАХОВКА от глубокого разворота, НЕ основной инструмент выхода.
                        # Только для крупных движений. Основные выходы: SL/трейлинг/импульс/break-even.
                        profit_sell = None
                        if pnl_percent >= 5.0:
                            _trend = getattr(self, '_trend_1h', {}).get(symbol, 'neutral')

                            # Пик должен быть минимум на 6% выше entry
                            highest = position.get('_highest_price', current_price)
                            if highest > entry_price * 1.06:
                        ...
                        if ie is not None and not ie.is_cooldown_active(symbol, 60):
                            if candles_1m and len(candles_1m) >= 5:
                                ...
                                if hasattr(self, '_pm') and symbol in self._pm._positions and getattr(self._pm._positions.get(symbol), 'direction', None) == 'short':
                                    ...
                                    try:
                                        _impulse_highest = position.get('_highest_price', current_price * (1 + pnl_percent / 100))
                                        impulse_result = ie.detect_impulse_exhaustion(
                                            candles_1m,
                                            entry_price=entry_price,
                                            current_pnl_pct=pnl_percent,
                                            vsa_result=vsa_for_impulse,
                                            highest_price=_impulse_highest
                                        )
                        ...
                        # ═══ ЖЕЛЕЗОБЕТОННЫЙ SL (первый приоритет) ──────────────
                        # Проверка напрямую, без ensemble, без SMART EXIT.
                        # SL двигается только вверх — если дошёл, выходим.
                        _sl_price_check = position.get('_sl_price', None)
                        if _sl_price_check is not None and current_price <= _sl_price_check:
                            should_sell = True
                            sell_reason = f"SL сработал: цена {current_price:.4f} ≤ SL {_sl_price_check:.4f}"
                            logger.info(f"🔴 [SL→EXIT] {symbol}: {sell_reason} (PnL={pnl_percent:+.2f}%)")

                        # 1) Early trail - если цена просела, но ensemble говорит держать
                        # Ранний трейлинг отключён (2026-07-11, решение пользователя)
                        ...
                        # 3) Импульсный выход с проверкой BTC контекста
                        # ⚡ Импульсный exit работает ТОЛЬКО когда BTC падает.
                        #   Если BTC растёт или в боковике — не выходим, даём позиции расти.
                        if not should_sell and impulse_exit_signal is not None:
                            _btc_regime_name = regime_result.get('regime', 'unknown')
                            _btc_1h_change = regime_result.get('btc_change_1h', 0)
                            _btc_4h_change = regime_result.get('btc_change_4h', 0)
                            _btc_bear_regimes = ('dump', 'distribution', 'bearish_side', 'panic', 'crash')
                            _btc_dropping = _btc_regime_name in _btc_bear_regimes or _btc_1h_change < -0.5 or _btc_4h_change < -1.0
                            if not _btc_dropping:
                                impulse_exit_signal = None
                                logger.info(f"🌡️ [IMPULSE→BTC-СКИП] {symbol}: BTC не падает (regime={_btc_regime_name} 1h={_btc_1h_change:+.2f}%) — импульсный exit отменён")
                            else:
                                logger.info(f"⚡ [IMPULSE→BTC-OK] {symbol}: BTC падает (regime={_btc_regime_name} 1h={_btc_1h_change:+.2f}%) — импульсный exit разрешён")

                        # ⚡ Structural Hold блокирует импульс, если модули подтверждают тренд
                        if not should_sell and impulse_exit_signal is not None:
                            try:
                                c5m_for_hold = mtf_candles_5m.get(symbol, [])
                                c1h_for_hold = mtf_candles_1h.get(symbol, [])
                                hold_check = de._check_structural_hold(
                                    symbol, entry_price, current_price, pnl_percent,
                                    highest_price=position.get('_highest_price', current_price * (1 + pnl_percent / 100)),
                                    candles_5m=c5m_for_hold,
                                    candles_1h=c1h_for_hold,
                                    cvd_data=_get_symbol_cvd(symbol)
                                )
                                if hold_check['approved']:
                                    impulse_exit_signal = None
                                    logger.info(f"🌡️ [IMPULSE→СКИП] {symbol}: структурный холд — "
                                                f"консенсус={hold_check['hold_confidence']}% "
                                                f"({hold_check['reason']})")
                                else:
                                    logger.info(f"⚡ [IMPULSE→ПРОПУСК] {symbol}: structural hold не прошёл — "
                                                f"{hold_check['hold_confidence']}% < 55 ({hold_check['reason']})")
                            except Exception as e:
                                logger.error(f"⚠️ [IMPULSE→STRUCTURAL_HOLD] {symbol}: ошибка: {e}")
                                # Fallback: старый метод (1H тренд)
                                ...
                                if c1h_for_impulse and len(c1h_for_impulse) >= 20:
                                    try:
                                        ...
                                        if ht in ('strong_bullish', 'bullish'):
                                            impulse_exit_signal = None
                                            logger.info(f"🌡️ [IMPULSE→СКИП] {symbol}: 1H={ht} - импульс заблокирован (fallback)")
                                    except Exception as e2:
                                        logger.error(f"⚠️ [IMPULSE→FALLBACK] {symbol}: ошибка 1H тренда: {e2}")
                        ...
                        if should_sell:
                            ...
                            # 🛡️ Если sell не удался - проверяем fetch_my_trades на бирже
                            # ☝️ Ордер мог исполниться в другом потоке или через order_manager
                            if result is None and symbol in self.positions:
                                logger.warning(f"⚠️ Sell {symbol} не удался. Проверяю fetch_my_trades...")
                                # 🩹 Пробуем найти реальный трейд на бирже
                                _real_sell = None
                                try:
                                    _raw = self.exchange.fetch_my_trades(symbol.split('/')[0] + '/USDT' if '/USDT' not in symbol else symbol, limit=10)
                                    _cutoff = (time.time() - 120) * 1000  # последние 2 минуты
                                    for _t in _raw:
                                        if _t.get('side', '').lower() == 'sell' and _t.get('timestamp', 0) > _cutoff:
                                            _real_sell = _t
                                            break
                                except Exception as _e:
                                    logger.debug(f"[POP] fetch_my_trades: {_e}")

                                if _real_sell:
                                    _avg_price = _real_sell.get('price', 0)
                                    _filled_qty = _real_sell.get('amount', 0)
                                    _order_id = _real_sell.get('order', 'unknown')
                                    logger.info(f"💡 [POP] {symbol}: найден реальный трейд! SELL {_filled_qty:.4f} @ ${_avg_price:.4f} (order={_order_id})")
                                    # Вычисляем PnL
                                    _entry = self.positions[symbol].get('entry_price', 0)
                                    _qty = self.positions[symbol].get('quantity', _filled_qty)
                                    _pnl = (_avg_price - _entry) * _qty
                                    _pnl_pct = ((_avg_price - _entry) / _entry * 100) if _entry > 0 else 0
                                    # Записываем в БД с реальной ценой
                                    db.add_trade(
                                        symbol=symbol,
                                        side='sell',
                                        price=float(_avg_price),
                                        quantity=float(_qty),
                                        pnl=float(_pnl),
                                        pnl_pct=float(_pnl_pct),
                                        exit_reason=sell_reason if sell_reason else 'pop_fallback',
                                        order_id=_order_id,
                                    )
                                    self.daily_pnl += _pnl
                                    self.available_capital += _entry * _qty + _pnl
                                    logger.info(f"✅ [POP] {symbol}: PnL={_pnl:+.2f}$ ({_pnl_pct:+.2f}%) | реальная цена")
                                else:
                                    logger.warning(f"⚠️ [POP] {symbol}: трейд на бирже не найден. Принудительно чищу (без PnL).")
                                # В любом случае чистим память
                                self.positions.pop(symbol, None)
                                db.remove_position(symbol)
                        else:
                ...
                if self._trading_blocked:
                    ...
                    if _max_new > 0:
                        ...
                        for _entry in _top_n:
                            ...
                            # 🛡️ Проверяем BTC блокировку перед любым входом
                            if getattr(self, '_btc_blocked', False):
                                logger.info(f"   🛑 [BTC-BLOCK] {_sym}: BTC режим запрещает входы, пропускаю")
                                continue
                            ...
                            if hasattr(self, '_pm') and hasattr(self, '_om') and hasattr(self, '_rm'):
                                try:
                                    ...
                                    _result = process_entry_decision(self, _sym, _dec, _price, btc_state_info)
                            ...
                            if not _integrated:
                                # SHORT через старый код не поддерживается — пропускаем
                                if getattr(_dec, 'side', 'long') == 'short':
                                    logger.warning(f"⏭️ [FALLBACK] {_sym}: SHORT через старый код невозможен, пропускаю")
                                    continue
                ...
                logger.info(f"📋 [SAVE] _save_real_balance() called at cycle #{getattr(self, '_cycle_count', 0)}")
    ...
    def monitor_system(self):
        ...
        from vote_tracker import get_tracker as _get_vt
        _prediction_check_interval = 0  # счётчик, раз в 5 минут
        
        while self.running:
            try:
                # 🧪 Проверка фоновых валидаций модулей (раз в 5 минут)
                _prediction_check_interval += 1
                if _prediction_check_interval >= 5:
                    _prediction_check_interval = 0
                    try:
                        # Собираем текущие цены наблюдаемых пар
                        prices = {}
                        for sym in list(self.positions.keys()) + list(self.price_cache.keys()):
                            if sym in self.price_cache and self.price_cache[sym] > 0:
                                prices[sym] = self.price_cache[sym]
                        _get_vt().check_pending_predictions(prices)
                    except Exception as _ve:
                        logger.debug(f"[VT] check_pending_predictions error: {_ve}")
    ...
    def _save_real_balance(self):
        ...
        logger.info(f"📋 [SAVE] _save_real_balance() entered")
        try:
            balance = self.exchange.fetch_balance()
            free_usdt = balance['free'].get('USDT', 0)
            ...
            # Собираем трейды с биржи для PnL-расчёта
            all_buy_trades = []
            recent_trades = []
            try:
                trade_syms = ['SOL/USDT', 'NEAR/USDT', 'BTC/USDT', 'FIL/USDT',
                ...
                for sym in trade_syms:
                    try:
                        trades = self.exchange.fetch_my_trades(sym, limit=100)
                        for t in trades:
                            entry = {
                                'symbol': t['symbol'],
                                'side': t['side'],
                        ...
                        logger.debug(f"[SAVE] fetch_my_trades({sym}): {e}")
                        pass
            except Exception as e:
                logger.debug(f"[SAVE] fetch_my_trades outer: {e}")
