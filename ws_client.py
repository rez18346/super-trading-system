...
class BybitWebSocketClient:
    ...
    def get_symbol_cvd_summary(self, symbol: str, n_minutes: int = 30) -> Optional[Dict]:
        ...
        # Считаем волатильность объёма: ср. объём за минуту vs последние 5 минут
        # Если объём упал — CVD легко обмануть
        vol_per_min = total_vol_usd / max(len(recent), 1)
        if len(recent) >= 5:
            recent5_usd = sum(m['buy_usd'] + m['sell_usd'] for m in minute_data[-5:])
            vol_last5 = recent5_usd / 5
        else:
            vol_last5 = vol_per_min
        
        # volume_ratio: 1.0 = средний, <0.5 = объём упал в 2+ раза, >2 = спайк
        volume_ratio = vol_last5 / max(vol_per_min, 1)
...
class CVDCollector:
    ...
    def __init__(self, max_minutes: int = 1440):
        ...
        # Если CSV пуст или старый — загружаем историю через REST
        try:
            if len(self.minutes) < 30:  # меньше 30 минут данных
                self._load_public_trades_history()
        except Exception as e:
            logger.debug(f"[CVD] history load error: {e}")
    ...
    def _load_public_trades_history(self, hours: int = 6):
        """Загрузить исторические publicTrade через REST API Bybit (за N часов)."""
        try:
            import ccxt
            ...
            # Торгуем только USDT пары (все 19 символов)
            symbols = ['BTC', 'ETH', 'SOL', 'XRP', 'ADA', 'AVAX', 'DOT', 'DOGE', 'LINK',
                       'ATOM', 'ALGO', 'EGLD', 'APT', 'ARB', 'ROSE', 'MNT', 'FIL', 'NEAR', 'OP']
            
            total_trades = 0
            # Берём топ-5 по объёму, остальные через WS догонят
            active = ['BTC', 'ETH', 'SOL', 'XRP', 'ADA']
            
            for sym in active:
                try:
                    trades = exchange.fetch_public_trades(sym + '/USDT', since=since_ms, limit=1000)
                    for t in trades:
                        self.add_trade(float(t['price']), float(t['amount']), t['side'], int(t['timestamp']))
                    total_trades += len(trades)
                    if len(trades) < 1000:
                        continue  # все данные за период получены
                except Exception as e:
                    logger.debug(f"[CVD] history {sym}: {e}")
            
            logger.info(f"[CVD] загружено {total_trades} исторических сделок за {h}ч")
        except ImportError:
            logger.debug("[CVD] ccxt не найден, history пропущен")
        except Exception as e:
            logger.debug(f"[CVD] history error: {e}")
