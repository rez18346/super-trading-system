...
def _get_config():
    ...
    Формат файла (JSON):
    {
      "enabled": true,                // 🛑 false = импульсный exit отключён
    ...
    """
    defaults = {
        'enabled': True,               # 🛑 false = полностью отключить импульс
...
def is_impulse_enabled() -> bool:
    """Проверить, включён ли импульсный exit."""
    cfg = _get_config()
...
def detect_impulse_exhaustion(candles_1m: List[Dict], 
                                ...
                                vsa_result: Optional[Dict] = None,
                                highest_price: float = None) -> ImpulseResult:
    """
    ...
    # 🛑 Отключаемый флаг (enabled=false = импульс не работает)
    if not is_impulse_enabled():
        sym_log = candles_1m[0].get('s', '') or ''
        now = time.time()
        _lk = f"{sym_log}_disabled_log"
        if _lk not in _LAST_IMPULSE_TRIGGER or now - _LAST_IMPULSE_TRIGGER[_lk] > 3600:
            _LAST_IMPULSE_TRIGGER[_lk] = now
            logger.info(f"🌡️ [IMPULSE] импульсный exit отключён (enabled=false)")
        return ImpulseResult(
            score=0, exhaustion=False,
            volume_divergence=0.0, body_trend=0.0,
            wick_rejection=False,
            signal='neutral', detail='импульс отключён'
        )
    ...
    # 🛡️ Дополнительный фильтр: импульсный exit только если цена ОТСТУПИЛА
    #     от максимума >0.3%. Без этого импульс режет на каждом микро-дыхании.
    if highest_price is not None and current_pnl_pct is not None:
        current_price = entry_price * (1 + current_pnl_pct / 100) if entry_price else None
        if current_price and highest_price > current_price:
            ...
            if drop_from_high < 0.3:
                return ImpulseResult(
                    ...
                    wick_rejection=False,
                    signal='hold',
                    detail=f'цене нет 0.3% отката от пика (drop={drop_from_high:.2f}%)'
                )
    ...
    if len(volumes) >= 6:
        ...
        # Сильное падение объёма — порог повышен с 50% до 60%
        if vol_prev3 > 0 and vol_last3 < vol_prev3 * (1 - strong_vol_drop * 1.2):
            score_components.append(('vol_crash', 90))
    
    # Резкое сокращение тела — ⚠️ ОТКЛЮЧЕНО: давало ложные срабатывания
    #   на каждом микро-дыхании рынка (score=85-100 каждые 2 мин)
    #   Включить при тестах: заменить pass на условие выше
    pass  # body_collapse временно отключён
    ...
    # ─── 6a. Ценовой фильтр: если нет отката от пика — не выходим
    #     (дублирует проверку выше, но для случая когда highest_price не передан)
    
    # ─── 7. Проверка: нужно достаточно подтверждений
