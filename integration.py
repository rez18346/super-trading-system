...
def process_entry_decision(trader_self, symbol: str, decision: Any,
    ...
    if max_usd is None:
        logger.warning(f"⏭️ [PM] {symbol}: adjust_position_size вернул None, пропускаю")
        return None
...
def process_exit_decision(trader_self, symbol: str, exit_reason: str,
    ...
    if pos.direction != Direction.SHORT:
        if total_asset < 0.000001:
            ...
            # 🩹 Проверяем fetch_my_trades — может уже продано по реальной цене
            _exit_price = current_price
            try:
                _raw = trader_self.exchange.fetch_my_trades(symbol, limit=5)
                _cutoff = (time.time() - 120) * 1000
                for _t in _raw:
                    if _t.get('side', '').lower() == 'sell' and _t.get('timestamp', 0) > _cutoff:
                        _exit_price = _t.get('price', current_price)
                        logger.info(f"💡 [EXIT] {symbol}: реальный exit по ${_exit_price:.4f}")
                        break
            except Exception:
                pass
            pm.close_position(symbol, _exit_price, f"нет_на_бирже:{exit_reason}")
            return None

        MIN_VALUE = 1.0
        if pos_value < MIN_VALUE:
            logger.warning(f"⏭️ [EXIT] {symbol}: пыль ${pos_value:.2f}")
            # 🩹 Проверяем fetch_my_trades — может не пыль а недогрузка
            _exit_price = current_price
            try:
                _raw = trader_self.exchange.fetch_my_trades(symbol, limit=5)
                _cutoff = (time.time() - 120) * 1000
                ...
                pass
            pm.close_position(symbol, _exit_price, f"пыль:{exit_reason}")
            return None
    ...
    if order_result.success and order_result.order:
        ...
        if order.is_filled:
            ...
            # 🩹 Ордер отправлен, но не исполнен
            # НЕ закрываем позицию в PM и НЕ пишем trade_history — дождёмся
            # синхронизации в следующем цикле, когда ордер исполнится.
            logger.info(f"⏳ [EXIT] {symbol}: ордер {order.status} — не исполнен, жду синхронизации")
            # Всё равно возвращаем TradeOutcome, чтобы трейдер не делал POP
            # (позиция остаётся в PM, следующий sync подхватит исполнение)
            return TradeOutcome(
                symbol=symbol,
                direction=pos.direction,
                entry_price=pos.entry_price,
                exit_price=order.avg_price if order.avg_price else current_price,
                quantity=safe_qty,
                pnl_pct=pos.pnl_pct,
                pnl_usd=pos.pnl_usd,
    ...
    # 🩹 FIX: если ордер не ушёл — проверяем fetch_my_trades на бирже
    # (ордер мог исполниться в другом потоке или через order_manager)
    real_trades = []
    try:
        if hasattr(trader_self, 'exchange') and trader_self.exchange:
            raw_trades = trader_self.exchange.fetch_my_trades(symbol, limit=5)
            base = symbol.split('/')[0]
            # Ищем sell-трейд за последние 30 секунд
            recent_cutoff = (time.time() - 30) * 1000  # в ms для Bybit timestamp
            for t in raw_trades:
                if t.get('side', '').lower() == 'sell' and t.get('timestamp', 0) > recent_cutoff:
                    real_trades.append(t)
    except Exception as e:
        logger.debug(f"[EXIT] {symbol}: fetch_my_trades не удался: {e}")

    if real_trades:
        # 🎉 Ордер всё-таки исполнился — берём реальную цену
        total_qty = sum(t.get('amount', 0) for t in real_trades)
        total_cost = sum(t.get('amount', 0) * t.get('price', 0) for t in real_trades)
        avg_price = total_cost / total_qty if total_qty > 0 else current_price
        logger.info(f"💡 [EXIT] {symbol}: найден реальный трейд, avg_price=${avg_price:.4f} (qty={total_qty:.4f})")
        outcome = pm.close_position(symbol, avg_price, exit_reason)
        if outcome:
            logger.info(f"✅ [EXIT] {symbol}: PnL={outcome.pnl_usd:+.2f}$ ({outcome.pnl_pct:+.2f}%) | {exit_reason} (реальная цена)")
            return outcome

    logger.warning(f"❌ [EXIT] {symbol}: ордер не отправлен — {order_result.error if hasattr(order_result, 'error') else 'неизвестная ошибка'}")
    return None
