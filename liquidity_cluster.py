...
logger = logging.getLogger('liquidity_cluster')
...
class LiquidityCluster:
    ...
    def __init__(self, window: int = 48):
        ...
        self._last_state: Optional[LiquidityState] = None
    ...
    def evaluate(self, candles_5m: List, current_price: float,
        ...
        # ── OI бонус (Open Interest + уровни ликвидаций) ────────────────
        # Использует collect_oi.py — сбор OI по 19 монетам, расчёт heat,
        # уровни ликвидаций. Только для спот-символов USDT.
        if '/USDT' in symbol:
            try:
                _oi = OICollector()
                if os.path.exists(OI_PATH):
                    with open(OI_PATH) as _f:
                        _oi_data = json.load(_f)
                    _oi_heat = _oi_data.get(symbol, {}).get('heat', 0)
                    if _oi_heat == 1:
                        score = min(100, score + 8)
                        detail += f' 🔥1'
                    elif _oi_heat == 2:
                        ...
                        # heat=3 → +25, если цена в зоне ликвидации → +35
                        _oi_extra = 25
                        _liq_zones = _oi_data.get(symbol, {}).get('liquidation_zones', [])
                        if any(zone[0] <= current_price <= zone[1] for zone in _liq_zones):
                            _oi_extra = 35
                            detail += f' 🔥⚠️'
                        score = min(100, score + _oi_extra)
                        detail += f' 🔥{_oi_heat}'
            except Exception as _oi_e:
                logger.debug(f"[OI] {symbol}: {_oi_e}")
    ...
    def _calc_score(self, closes, highs, lows, volumes,
        ...
        # ── 0. Volume sanity: мёртвый рынок — не даём высокий score ────────
        if len(volumes) >= 48:
            recent_vol = sum(volumes[-5:]) / 5.0
            hist_vol = sum(volumes[-48:]) / 48.0
            if hist_vol > 0:
                vol_ratio = recent_vol / hist_vol
                if vol_ratio < 0.3:
                    # Рынок умер: 5-свечной объём <30% от исторического
                    # Любой score > 55 — ловушка на тонком рынке
                    score = min(55, score + 5)
                    logger.debug(f"[Liq] Volume cap: vol_ratio={vol_ratio:.2f} < 0.3 — score capped at {score}")
                    # Всё, дальше не считаем — рынок мёртв
                    return int(score)
        ...
        # ── 4. Breakout Trap — ИСПРАВЛЕНО ───────────────────────────────
        trap = self._detect_breakout_trap(highs[-20:], lows[-20:], closes[-20:],
                                           volumes[-20:] if len(volumes) >= 20 else [],
                                           vah, val)
        if trap == 'bullish' or trap == 'bearish':
            # Ложный пробой VAH/VAL с возвратом — цена не смогла закрепиться.
            # Это не сигнал к развороту, а признак слабости/ловушки.
            score = max(30, score - 15)
        elif trap == 'fake_breakout':
            # Двойной пробой VAH с возвратом + объём — классическая ловушка
            score = max(20, score - 25)

        # ── Context: bearish срез ──────────────────────────────────────────
        # Если bearish_context активен — срезаем накопленный score
        if bearish_context:
            score = min(score, 55)  # не выше 55 при медвежьем контексте
    ...
    def get_premium_discount(self, current_price: float) -> Dict:
        """
        🏷️ Оценка Premium/Discount цены относительно SMC-зон (OB/FVG/VWAP).

        Returns:
            premium_pct: на сколько % цена выше Discount-зоны
            discount_pct: на сколько % цена ниже Premium-зоны
            signal: 'premium' | 'discount' | 'fair'
            nearest_bullish_ob_price: ближайший бычий OB снизу (или None)
            nearest_bearish_ob_price: ближайший медвежий OB сверху (или None)
            fvg_discount_below: FVG ниже цены (или None)
            fvg_premium_above: FVG выше цены (или None)
            vwap_deviation_pct: отклонение от VWAP 1H (если есть)
        """
        result = {
            'premium_pct': 0.0,
            'discount_pct': 0.0,
            'signal': 'fair',
            'nearest_bullish_ob_price': None,
            'nearest_bearish_ob_price': None,
            'fvg_discount_below': None,
            'fvg_premium_above': None,
        ...
        # ── 1. Ищем ближайший бычий OB ниже цены (Discount-зона) ──────────
        bullish_obs_below = [
            ob for ob in state.order_blocks
            if ob.direction == 'bullish' and not ob.mitigated and ob.price_high < current_price
        ]
        if bullish_obs_below:
            nearest = max(bullish_obs_below, key=lambda ob: ob.price_high)
            result['nearest_bullish_ob_price'] = nearest.price_high
            dist_pct = (current_price - nearest.price_high) / nearest.price_high * 100
            result['premium_pct'] = max(0, dist_pct)

        # ── 2. Ищем ближайший медвежий OB выше цены (Premium-зона) ───────
        bearish_obs_above = [
            ob for ob in state.order_blocks
            if ob.direction == 'bearish' and not ob.mitigated and ob.price_low > current_price
        ]
        if bearish_obs_above:
            nearest = min(bearish_obs_above, key=lambda ob: ob.price_low)
            result['nearest_bearish_ob_price'] = nearest.price_low
            dist_pct = (nearest.price_low - current_price) / current_price * 100
            result['discount_pct'] = max(0, dist_pct)

        # ── 3. FVG анализ ───────────────────────────────────────────────
        if state.fvg_below:
            closest_fvg = max(state.fvg_below, key=lambda f: f[1])
            result['fvg_discount_below'] = closest_fvg
        if state.fvg_above:
            closest_fvg = min(state.fvg_above, key=lambda f: f[0])
            result['fvg_premium_above'] = closest_fvg

        # ── 4. VWAP отклонение ───────────────────────────────────────────
        if state.vwap_1h and state.vwap_1h > 0:
            vwap_dev = (current_price - state.vwap_1h) / state.vwap_1h * 100
            result['vwap_deviation_pct'] = round(vwap_dev, 2)

        # ── 5. Итоговый сигнал ───────────────────────────────────────────
        premium_pct = result['premium_pct']
        discount_pct = result['discount_pct']
        vwap_dev = result['vwap_deviation_pct']

        # Premium: цена выше OB > 3% ИЛИ выше VWAP > 2%
        is_premium = premium_pct > 3.0 or (vwap_dev is not None and vwap_dev > 2.0)
        # Discount: цена ниже VWAP > 1.5% ИЛИ у FVG снизу
        is_discount = (vwap_dev is not None and vwap_dev < -1.5) or (state.fvg_below and abs(current_price - state.fvg_below[-1][0]) / current_price < 0.02)
