...
def parse_votes() -> dict:
    ...
    for line in reversed(lines):
        ...
        # Пропускаем не-DE→ и не-SHORT→ строки
        if 'DE→' not in line and 'SHORT→' not in line:
            continue

        # DE→ для лонгов, SHORT→ для шортов
        sym_m = re.search(r'\[(DE|SHORT)→(HOLD|BUY|SELL|ENTER|CANDIDATE)\] (\w+)/USDT:', line)
        if not sym_m:
            # Новый формат: DE→HOLD SOL/USDT:
            sym_m = re.search(r'(DE|SHORT)→(HOLD|BUY|SELL|ENTER|CANDIDATE) (\w+)/USDT:', line)
        if not sym_m:
            continue

        entry_type = sym_m.group(1)  # DE или SHORT
        raw_sym = sym_m.group(3)
        sym = raw_sym + '/USDT'
        if sym in result:
            continue  # уже нашли

        signal = sym_m.group(2)
        direction = 'SHORT' if entry_type == 'SHORT' else 'LONG'
        ...
        if pf:
            ...
            ls_match = re.search(r'L=(-?[\d.]+)\s+S=(-?[\d.]+)', line)
            long_score = int(float(ls_match.group(1))) if ls_match else 0
            short_score = int(float(ls_match.group(2))) if ls_match else 0
            ...
            # Если score от decision_engine: L=.. S=..
            if score == -1 and ls_match:
