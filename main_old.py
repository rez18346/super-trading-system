...
# Глобальный флаг: _check_pid_file() вызывается только один раз
# Нужно, потому что модули при импорте могут триггерить log и повторный вызов
_PID_CHECKED = [False]


def _check_pid_file():
    """Единственный страж: PID-файл в data/trader.pid.
    
    Вызывается один раз при старте. Повторные вызовы игнорируются.
    
    
    Логика:
      1. Если PID-файл существует и процесс в нём жив и это main.py —
         выходим (дубль не нужен).
      2. Если PID мёртв или это не main.py — убиваем процесс на порту (если есть)
         и занимаем место.
      3. Записываем свой PID.
    """
    if _PID_CHECKED[0]:
        return  # уже проверяли, игнорируем повторный вызов
    _PID_CHECKED[0] = True
    ...
    # ── 2. Если PID-файл уже указывает на нас — всё ок, выходим ──────────
    if old_pid == current_pid:
        return

    # ── 3. Проверка: жив ли старый процесс и это main.py? ─────────────────
    if old_pid:
        try:
            os.kill(old_pid, 0)  # проверка существования (сигнал 0)
            try:
            ...
            if is_main:
                # Наш main.py уже работает — выходим
                _log.warning(f"🔄 Старый main.py PID={old_pid} жив — выхожу, дубль не нужен")
                sys.exit(0)
            else:
                # Чужой процесс — убиваем
                _log.warning(f"⚰️ Чужой PID={old_pid} ({cmdline[:60]}), убиваю...")
                os.kill(old_pid, 9)
                time.sleep(0.5)
        except ProcessLookupError:
            _log.info(f"📝 Старый PID={old_pid} мёртв — перезаписываю")

    # ── 4. Освобождаем порт 8765 от сирот (control_api без трейдера) ─────
    _free_port_8765()

    # ── 5. Записываем свой PID ────────────────────────────────────────────
    _write_pid(current_pid)


def _free_port_8765():
    """Принудительно освободить порт 8765 от процессов, которые НЕ main.py.
    
    Используется только ss (не lsof). Не убивает себя.
    """
    import subprocess
    try:
        ...
        for match in re.finditer(r'pid=(\d+)', result.stdout):
            ...
            try:
                ...
                if 'main.py' in cmdline:
                    _log.info(f"ℹ️  PID {pid} на порту 8763 — это main.py, не трогаю")
                    continue
            except Exception:
                pass
            try:
                os.kill(pid, 9)
                _log.info(f"📌 Сирота PID {pid} убит на порту 8763")
                time.sleep(0.3)
            except ProcessLookupError:
                pass
    except Exception as e:
        _log.debug(f"free_port_8765: {e}")


def _write_pid(pid: int):
    """Атомарная запись PID в файл."""
    import tempfile
    tmp = PID_FILE + '.tmp'
    with open(tmp, 'w') as f:
        f.write(str(pid))
    os.rename(tmp, PID_FILE)
    _log.info(f"📝 PID-файл: {pid}")
