...
def import_partitions_create(cur, conn):
    ...
    # Сбрасываем транзакцию если она в ошибке
    try:
    ...
    now = datetime.now()
    for offset in [0, 1, 2]:
        conn.rollback()  # чистое состояние для каждой итерации
        ...
        try:
            ...
            print("[db_pg] WARN check " + part_name + ": " + str(e))
            continue
        if dt.month == 12:
...
def init_db(db_path=None):
    ...
    def _init(conn):
        ...
        if has_trades:
            ...
            # Сбрасываем транзакцию перед созданием партиций
            conn.rollback()
            cur = conn.cursor()
