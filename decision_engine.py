...
class DecisionEngine:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.reentry_cooldown = 3600  # 1 час (базовый кулдаун после выхода)
        ...
        # 🆕 Кэш для determine_direction (защита от двойного вызова модулей)
        self._dir_cache: Dict[str, dict] = {}  # key: symbol, value: {time, ensemble_result}
        self._dir_cache_ttl = 10  # 10 секунд
    ...
    def _evaluate_exit_ensemble(self, symbol: str,
        ...
        try:
            if self._ml_advisor and self._ml_advisor.is_trained:
                ...
                if candles_5m and len(candles_5m) > 10:
                    try:
                        ...
                        if adv_df is not None and len(adv_df) > 0:
                            ...
                            adv_df = adv_df.rename(columns={k:v for k,v in _rename.items() if k in adv_df.columns})
                    except Exception:
                        adv_df = None
                ...
                if adv['decision'] == 'GOOD':
                    ...
                    # WEAK = 45-65 баллов в зависимости от confidence
                    scores['advisor']['score'] = min(65, 35 + int(adv['confidence'] * 60))
        ...
        try:
            if self._liquidity:
                liq = self._liquidity.evaluate(candles_5m or [], current_price, None, None, symbol=symbol)
        ...
        # ═══ CVD (10%) — реальный CVD из WebSocket (поток buy/sell) ──────
        try:
            if cvd_data and cvd_data.get('minutes', 0) >= 2:
                ...
                cvd_net = cvd_data.get('cvd_net', 0)
                
                # CVD от WebSocket уже реальный: buy/sell объёмы напрямую с биржи
                # База: buy_pct (50%=нейтрально, >55%=бычий, <45%=медвежий)
                cvd_score = 50 + (buy_pct - 50) * 2.0
                
                # Коррекция по тренду (WebSocket trend честнее — сравнивает 10мин vs 20мин)
                if trend == 'bullish':
                    cvd_score += 15
                elif trend == 'bearish':
                    cvd_score -= 15
                
                # Коррекция по cvd_net (чистый поток)
                # Если cvd_net > 0 — больше купили, чем продали (бычий)
                # Если cvd_net < 0 — больше продали (медвежий)
                # Нормализуем: чем больше абсолютное значение, тем сильнее сигнал
                cvd_net_usd = abs(cvd_net)
                if cvd_net_usd > 1000:
                    if cvd_net > 0:
                        cvd_score += 10
                    else:
                        cvd_score -= 10
                elif cvd_net_usd > 100:
                    if cvd_net > 0:
                        cvd_score += 5
                    else:
                        cvd_score -= 5
                
                # 🛡 Штраф за малый объём: CVD легко обмануть на тонком рынке
                # volume_ratio = объём последних 5 мин / средний объём за 30 мин
                # 1.0 = норма, <0.5 = вдвое меньше, <0.3 = критически мало
                volume_ratio = cvd_data.get('volume_ratio', 1.0)
                if volume_ratio < 0.5:
                    # Объём упал в 2+ раза — штраф линейно
                    vol_penalty = int((0.5 - volume_ratio) * 40)  # max ~20 баллов
                    cvd_score -= vol_penalty
                    cvd_vol_note = f' 📉V={volume_ratio:.1f}x -{vol_penalty}'
                elif volume_ratio <= 3:
                    # Нормальный или чуть выше — без штрафа
                    cvd_vol_note = ''
                else:
                    # Спайк объёма (>3x) — подтверждает CVD, небольшой бонус
                    cvd_score += 5
                    cvd_vol_note = ' 📈V-spike'
                
                cvd_score = max(10, min(100, cvd_score))
                scores['cvd']['score'] = cvd_score
                scores['cvd']['detail'] = f'trend={trend} buy={buy_pct:.0f}% ratio={cvd_ratio:.2f} net={cvd_net:.0f}{cvd_vol_note}'
                logger.debug(f"[CVD] {symbol}: score={cvd_score:.0f} — {scores['cvd']['detail']}")
            else:
                scores['cvd']['score'] = 50
                scores['cvd']['detail'] = 'no_data'
        except Exception as e:
            logger.debug(f"[CVD] {symbol}: {e}")
            scores['cvd']['score'] = 50
    ...
    def decide_exit(self, symbol: str, entry_price: float, current_price: float,
        ...
        # ─── 1. SL - стоп-лосс ──────────────────────────────────────────
        # 🎯 SL с проверкой STRUCTURAL HOLD:
        #    Если цена пробила SL, но все модули говорят «bullish, это откат» -
        #    расширяем SL на 30-50% вместо выхода.
        #    Если структура weak/neutral — exit как обычно.
        if pnl_pct <= -_effective_sl:
            # ⚡ Structural Hold Check: модули говорят «держать»?
            _sl_ensemble = None
            if candles_1m and len(candles_1m) >= 5:
                _sl_ensemble = self._check_structural_hold(
                    symbol, entry_price, current_price, pnl_pct,
                    highest_price=highest_price, candles_5m=candles_1m,
                    candles_1h=candles_1h, cvd_data=cvd_data
                )
            
            if _sl_ensemble and _sl_ensemble['approved']:
                # Структура сильная — расширяем SL
                widen_pct = _sl_ensemble.get('widen_pct', 0.3)
                new_sl_pct = min(_effective_sl * (1 + widen_pct), max_sl_pct)
                action = 'расширение 30%' if widen_pct < 0.5 else 'полное (50%)'
                reason = (f"SL structural hold: {action} "
                          f"(strength={_sl_ensemble['hold_confidence']}% "
                          f"[{_sl_ensemble.get('reason','')}]"
                          f" | SL {_effective_sl:.2f}%→{new_sl_pct:.2f}%")
                logger.info(f"🏗️ [STRUCTURAL HOLD] {symbol}: {reason}")
                return Decision(
                    symbol, 'hold', side=side, priority=85,
                    reason=reason,
                    signal_type=SignalType.HOLD,
                    sl_price=entry_price * (1 - new_sl_pct / 100.0),
                    exit_override='hold_widen_sl',
                    exit_vote=_sl_ensemble
                )
            
            # Структура слабая или нет данных — безусловный выход
    ...
    def _check_structural_hold(self, symbol: str, entry_price: float,
                                ...
                                cvd_data: Optional[Dict] = None) -> Dict:
        """
        Оценка СТРУКТУРЫ ДВИЖЕНИЯ для SL.
        Если модули в консенсусе «тренд сильный, это откат» —
        возвращает approved=True с расширением SL.

        Использует:
        - Liq (25%): структура ликвидности/накопление
        - VSA (25%): качество движения, дивергенция
        - Advisor (20%): уверенность в тренде
        - CVD (15%): поток агрессивных сделок
        - 1H тренд (15%): старший таймфрейм

        hold_confidence: общая уверенность в структуре (0-100)
        approved: True если confidence >= 55 (расширяем SL)
        widen_pct: 0.3 при confidence 55-64, 0.5 при >=65
        """
        result = {
            'hold_confidence': 0,
            'approved': False,
            ...
            'votes': {},
            'reason': '',
            'method': 'structural_hold'
        }

        if not candles_5m or len(candles_5m) < 5:
            result['reason'] = 'нет данных'
            return result
        ...
        # ═══ 1. VSA (25%) — качество движения ═══════════════════════════
        try:
            from vsa_analyzer import analyze_volume_spread
            vsa = analyze_volume_spread(candles_5m)
            if vsa.signal == 'bullish':
                vsa_pts = 60 + vsa.strength * 40  # 60-100
                votes['vsa'] = vsa_pts
                vote_details.append(f"VSA={vsa_pts}(bullish)")
            elif vsa.signal == 'neutral' and vsa.strength > 0.1:
                vsa_pts = 50 + vsa.strength * 30  # 50-80
                votes['vsa'] = vsa_pts
                vote_details.append(f"VSA={vsa_pts}(neutral+strength)")
            else:
                vsa_pts = 40 - vsa.strength * 30  # 10-40
                votes['vsa'] = vsa_pts
                vote_details.append(f"VSA={vsa_pts}(bearish)")
            score += vsa_pts * 0.25
        except Exception as e:
            votes['vsa'] = 50
            score += 50 * 0.25
            vote_details.append(f"VSA=50(err)")

        # ═══ 2. Liq (25%) — структура ликвидности ════════════════════
        try:
            from decision_engine import _evaluate_entry_ensemble
            # Используем Liq из entry — если он показывает накопление
            # это сильный сигнал что откат не разворот
            liq_score = 50
            if hasattr(self, '_liquidity') and self._liquidity:
                liq = self._liquidity.evaluate(candles_5m or [], current_price,
                                               None, None, symbol=symbol)
                if isinstance(liq, dict):
                    liq_score = liq.get('score', 50)
                else:
                    liq_score = getattr(liq, 'score', 50)
            votes['liquidity'] = liq_score
            vote_details.append(f"Liq={liq_score}")
            score += liq_score * 0.25
        except Exception:
            votes['liquidity'] = 50
            score += 50 * 0.25
            vote_details.append(f"Liq=50(err)")

        # ═══ 3. Advisor (20%) — уверенность в тренде ══════════════════
        try:
            adv_score = 50
            if self._ml_advisor and self._ml_advisor.is_trained:
                adv = self._ml_advisor.evaluate(
                    symbol, current_price, 50, 'neutral', 50
                )
                if adv['decision'] == 'GOOD':
                    adv_score = 60 + adv['confidence'] * 40  # 60-100
                elif adv['decision'] == 'WEAK':
                    adv_score = 40 + adv['confidence'] * 30  # 40-70
                else:
                    adv_score = 30 + adv['confidence'] * 20  # 30-50
            votes['advisor'] = adv_score
            vote_details.append(f"Adv={adv_score}")
            score += adv_score * 0.20
        except Exception:
            votes['advisor'] = 50
            score += 50 * 0.20
            vote_details.append(f"Adv=50(err)")

        # ═══ 4. CVD (15%) — поток агрессивных сделок ═══════════════════
        try:
            if cvd_data:
                buy_pct = cvd_data.get('buy_pct', 50)
                cvd_trend = cvd_data.get('trend', 0)
                # buy_pct > 55 + положительный тренд = сильное накопление
                if buy_pct > 55 and cvd_trend > 0:
                    cvd_score = min(100, 50 + (buy_pct - 50) * 3 + cvd_trend * 5)
                elif buy_pct < 45:
                    cvd_score = max(20, 50 - (50 - buy_pct) * 2)
                else:
                    cvd_score = 50
                
                # 🛡 Штраф за малый объём (как в основном CVD)
                volume_ratio = cvd_data.get('volume_ratio', 1.0)
                if volume_ratio < 0.5:
                    vol_penalty = int((0.5 - volume_ratio) * 40)
                    cvd_score -= vol_penalty
                    cvd_vol_note = f' V={volume_ratio:.1f}x'
                elif volume_ratio > 3:
                    cvd_score += 5
                    cvd_vol_note = ' V-spike'
                else:
                ...
                votes['cvd'] = cvd_score
                vote_details.append(f"CVD={cvd_score}(buy={buy_pct:.0f}%{cvd_vol_note})")
                score += cvd_score * 0.15
            else:
                votes['cvd'] = 50
                score += 50 * 0.15
                vote_details.append(f"CVD=50(no_data)")
        except Exception:
            votes['cvd'] = 50
            score += 50 * 0.15
            vote_details.append(f"CVD=50(err)")

        # ═══ 5. 1H тренд (15%) — старший таймфрейм ════════════════════
        try:
            _1h_score = 50
            if candles_1h and len(candles_1h) >= 20:
                _1h_t = self._calc_trend_from_candles(candles_1h)
                if _1h_t in ('strong_bullish',):
                    _1h_score = 85
                    vote_details.append(f"1H=85(strong_bullish)")
                elif _1h_t in ('bullish',):
                    _1h_score = 70
                    vote_details.append(f"1H=70(bullish)")
                elif _1h_t in ('neutral',):
                    _1h_score = 50
                    vote_details.append(f"1H=50(neutral)")
                else:
                    _1h_score = 30
                    vote_details.append(f"1H=30(bearish)")
            votes['tf'] = _1h_score
            score += _1h_score * 0.15
        except Exception:
            votes['tf'] = 50
            score += 50 * 0.15
            vote_details.append(f"1H=50(err)")

        result['hold_confidence'] = int(score)
        result['votes'] = votes
        result['reason'] = ', '.join(vote_details)

        # ⚡ Решение: структура сильная?
        if score >= 65:
            result['approved'] = True
            result['widen_pct'] = 0.5  # полное расширение 50%
        elif score >= 55:
            result['approved'] = True
            result['widen_pct'] = 0.3  # расширение 30%
        else:
    ...
    def decide_entry(self, symbol: str, confidence: float, trend: str,
        ...
        # 0. Лимит позиций (самая жёсткая блокировка, ДО всего)
        if current_positions_count > max_positions:
            return Decision(symbol, 'hold', side=side,
                reason=f"Максимум {max_positions} позиций")
        ...
        if entry_checks['approved']:
            ...
            return Decision(
                symbol, 'hold', side=side,
                reason=entry_checks['reason'],
                metadata=entry_checks  # 🩹 FIX: всегда сохраняем голоса для SHORT-расчёта
            )
    ...
    def _get_entry_threshold(self, strong_votes: int = 0, liq_score: float = 0) -> float:
        """Адаптивный порог входа.

        Факторы снижения порога:
        - HMM CALM: -8 (риск-менее агрессивно)
        - Liquidity >= 75: -3
        - Liquidity >= 90: -5 (сигнал максимально уверенный)
        ...
        # Только Liquidity влияет на порог
        if liq_score >= 90:
            base -= 5
        elif liq_score >= 75:
            base -= 3
    ...
    def _evaluate_entry_ensemble(self, symbol: str, confidence: float,
        ...
        BASE_WEIGHTS = {
            'ml_v2': 0.00,  # отключён
            'advisor': 0.00,  # → veto-only (точность 42% — хуже монетки)
            'mtf': 0.00,
            'rsi_vol_btc': 0.00,
            'liquidity': 1.00,  # 🏆 единственный голосующий модуль (57.5% точности)
            'volume_vwap': 0.00,  # → veto-only (44%)
            'vsa': 0.00,  # → veto-only
            'cvd': 0.00,  # → veto-only
            'smc': 0.00,  # SMC — отдельный запрет, не взвешенный голос
        }
        # Liquidity — единственный голосующий модуль. Остальные → veto-штрафы.
        SHORT_WEIGHTS = {
            'ml_v2': 0.00,
            'advisor': 0.00,  # → veto-only
            'mtf': 0.00,
            'rsi_vol_btc': 0.00,
            'liquidity': 1.00,  # 🏆 единственный голосующий
            'volume_vwap': 0.00,  # → veto-only
            'vsa': 0.00,  # → veto-only
            'cvd': 0.00,  # → veto-only
            'smc': 0.00,  # SMC — отдельно
        }

        weights = SHORT_WEIGHTS if is_short else BASE_WEIGHTS

        scores = {}
        for k, w in weights.items():
            scores[k] = {'score': 50, 'weight': w, 'detail': 'N/A'}

        # 🧠 VoteTracker больше не меняет веса — только Liquidity голосует.
        # Оставлен на случай возврата к динамическим весам.
            ...
            if self._ml_advisor and self._ml_advisor.is_trained:
                ...
                if candles_5m and len(candles_5m) > 10:
                    try:
                        ...
                        adv_df = _pd.DataFrame(candles_5m)
                        if adv_df is not None and len(adv_df) > 0:
                            # Переименовываем короткие ключи (o/h/l/c/v) в полные названия
                            _rename = {'o':'open','h':'high','l':'low','c':'close','v':'volume'}
                            adv_df = adv_df.rename(columns={k:v for k,v in _rename.items() if k in adv_df.columns})
                    except Exception as _e:
        ...
        try:
            if self._liquidity:
                liq = self._liquidity.evaluate(candles_5m, current_price,
                                                candles_1h, candles_4h, symbol=symbol)
        ...
        # ═══ ГОЛОС 9: BTC SMC Filter (veto-штраф) 🆕 ════════════════════════
        try:
            from btc_smc_filter import BTCSmcFilter
            if not hasattr(self, '_btc_smc_filter') or self._btc_smc_filter is None:
                self._btc_smc_filter = BTCSmcFilter()
            
            # Собираем BTC свечи из переданных данных (если symbol == BTC/USDT)
            # Или используем кешированные глобальные BTC свечи из трейдера
            _c1h = None
            if symbol == 'BTC/USDT':
                _c1h = candles_1h if candles_1h is not None else (self._candles_1h if hasattr(self, '_candles_1h') else None)
            _c4h = None
            if symbol == 'BTC/USDT':
                _c4h = candles_4h if candles_4h is not None else (self._candles_4h if hasattr(self, '_candles_4h') else None)
            smc_result = self._btc_smc_filter.analyze(
                candles_15m=candles_5m if symbol == 'BTC/USDT' else None,
                candles_1h=_c1h,
                candles_4h=_c4h
            )
            
            smc_veto = smc_result.get('veto', 0)
            smc_direction = smc_result.get('direction', 'neutral')
            smc_emergency = smc_result.get('emergency', False)
            smc_reason = smc_result.get('reason', 'N/A')
            
            # Преобразуем veto (-15..0) в score (0..100)
            # veto=0 → score=50 (нейтрально)
            # veto=-7 → score=35 (медвежья структура)
            # veto=-15 → score=15 (сильная медвежья структура)
            smc_score = 50 + smc_veto  # veto=-7 → 43, veto=-15 → 35
            smc_score = max(10, min(100, smc_score))
            
            scores['smc'] = {
                'score': smc_score,
                'weight': 0,  # SMC — veto-штраф, не взвешенный
                'veto': smc_veto,
                'emergency': smc_emergency,
                'detail': f'{smc_reason} (veto={smc_veto})',
            }
            
            if smc_emergency:
                logger.warning(f"🚨 [SMC] EMERGENCY_CLOSE: BTC пробил уровень! {smc_reason}")
            if smc_veto != 0:
                logger.info(f"[SMC] {symbol}: veto={smc_veto:+d} dir={smc_direction} — {smc_reason}")
            elif symbol == 'BTC/USDT':
                logger.debug(f"[SMC] {symbol}: veto=0 dir=neutral — {smc_reason}")
                
        except Exception as e:
            scores['smc'] = {'score': 50, 'weight': 0, 'veto': 0, 'detail': f'error({e})'}
        ...
        # Версия для боковика: жёсткий штраф, цена у хая — риск.
        # Порог: 0.97 (цена ≥97% от хая → штраф), макс 25 pts на 1.0
        # Обновлено 2026-06-23: возврат к агрессивной защите (боковик BTC)
        if high_24h is not None and high_24h > 0 and current_price > 0:
            _ratio = current_price / high_24h
            if _ratio >= 0.97:
                _penalty = min(25, (_ratio - 0.97) * 833)  # 0.97→0, 1.0→25
        ...
        # ═══ PREMIUM/DISCOUNT FILTER — Smart Money зоны входа ════════════
        # Эксперт сказал: вход должен быть от OBs/FVGs, а не в Premium.
        # Если цена >3% выше ближайшего бычьего OB → это Premium zone → штраф
        # Если цена у/внутри OB или ниже VWAP → Discount zone → бонус
        _prem_disc = 0
        _prem_disc_reason = ''
        try:
            if self._liquidity:
                pd = self._liquidity.get_premium_discount(current_price)
                if pd['signal'] == 'premium':
                    # Premium: цена далеко от справедливой зоны — штраф
                    prem_pct = pd['premium_pct']
                    if prem_pct > 8:
                        _prem_disc = -35
                        _prem_disc_reason = f'PREMIUM({prem_pct:.1f}% выше OB)'
                    elif prem_pct > 5:
                        _prem_disc = -25
                        _prem_disc_reason = f'PREMIUM({prem_pct:.1f}% выше OB)'
                    else:
                        _prem_disc = -15
                        _prem_disc_reason = f'PREMIUM({prem_pct:.1f}% выше OB)'
                elif pd['signal'] == 'discount':
                    # Discount: цена в зоне справедливой стоимости — бонус
                    _prem_disc = 20
                    _prem_disc_reason = 'DISCOUNT(у OB/FVG)'
                else:
                    # Fair: нейтрально, но проверяем VWAP
                    vwap_dev = pd.get('vwap_deviation_pct')
                    if vwap_dev is not None:
                        if vwap_dev < -1.0:
                            _prem_disc = 10
                            _prem_disc_reason = f'FAIR~DISCOUNT(VWAP {vwap_dev:+.1f}%)'
                        elif vwap_dev > 1.0:
                            _prem_disc = -8
                            _prem_disc_reason = f'FAIR~PREMIUM(VWAP {vwap_dev:+.1f}%)'

                if _prem_disc != 0:
                    scores['_prem_disc'] = {'score': _prem_disc, 'weight': 1.0, 'detail': _prem_disc_reason}
                    logger.info(f"🏷️ [PREMIUM/DISCOUNT] {symbol}: {_prem_disc_reason} → {_prem_disc:+d}pts")
                else:
                    scores['_prem_disc'] = {'score': 0, 'weight': 1.0, 'detail': 'fair'}
        except Exception as _pd_e:
            logger.debug(f"[PREMIUM/DISCOUNT] {symbol}: {_pd_e}")
            scores['_prem_disc'] = {'score': 0, 'weight': 1.0}

        # ═══ ИТОГОВЫЙ СКОР ═════════════════════════════════════════════════
        # БАЗОВЫЙ score — только взвешенные модули (сейчас это Liquidity 1.0)
        # Бонусы (price, reversal) НЕ участвуют в проходе порога — только в сортировке
        _base_keys = [k for k in BASE_WEIGHTS.keys()]  # только модули из BASE_WEIGHTS
        base_score = sum(scores[k]['score'] * scores[k]['weight'] for k in _base_keys if k in scores)
        
        # Добавляем бонусы для финального score (используется для сортировки кандидатов)
        final_score = base_score + price_bonus + reversal_bonus + btc_bonus
        ...
        # 🏷️ Premium/Discount фильтр — штраф/бонус к финальному скору
        _prem_val = scores.get('_prem_disc', {}).get('score', 0)
        if _prem_val != 0:
            og_score = final_score
            final_score += _prem_val
            final_score = max(0, min(150, final_score))
            logger.info(f"🏷️ [PREM/DISC] {symbol}: {_prem_val:+d}pts ({og_score:.1f}→{final_score:.1f})")

        # SMC veto применяется к финальному скору (как RP — дельта, не взвешенный голос)
        _smc_veto = scores.get('smc', {}).get('veto', 0)
        if _smc_veto < 0:
            og_score = final_score
            final_score += _smc_veto  # veto=-7 → -7pts к score, veto=-15 → -15pts
            final_score = max(0, min(150, final_score))
            logger.debug(f"[SMC] {symbol}: veto={_smc_veto:+d}pts ({og_score:.1f}→{final_score:.1f}) — {scores['smc'].get('detail', '')}")

        # ═══ VSA/CVD VETO — штраф за консенсус (оба FOR = перегрев)
        _vsa_score = scores.get('vsa', {}).get('score', 50)
        _cvd_score = scores.get('cvd', {}).get('score', 50)
        _vsa_for = _vsa_score >= 60
        _cvd_for = _cvd_score >= 60
        _veto_vsa_cvd = 0
        if _vsa_for and _cvd_for:
            _veto_vsa_cvd = -15  # оба за — сильный перегрев
        elif _vsa_for or _cvd_for:
            _veto_vsa_cvd = -5   # один за — слабый сигнал
        if _veto_vsa_cvd < 0:
            og_score = final_score
            final_score += _veto_vsa_cvd
            final_score = max(0, min(150, final_score))
            logger.info(f"[VETO] {symbol}: VSA={_vsa_score:.0f} CVD={_cvd_score:.0f} штраф={_veto_vsa_cvd}pts ({og_score:.1f}→{final_score:.1f})")
        ...
        # Только Liquidity определяет силу сигнала
        strong_votes = 1 if scores['liquidity']['score'] >= 75 else 0
        ...
        # ═══ РЕШЕНИЕ ПО БАЗОВОМУ SCORE — бонусы влияют ТОЛЬКО на сортировку между кандидатами, не на проход порога
        passes_threshold = base_score >= threshold

        logger.debug(f"[threshold] symbol={symbol} threshold={threshold} strong={strong_votes} liq={scores['liquidity']['score']} adv={scores['advisor']['score']} vsa={scores['vsa']['score']} base_score={base_score:.0f} final_score={final_score:.0f} bonus=p:{price_bonus:.0f}+r:{reversal_bonus:.0f}+b:{btc_bonus:.0f}")
        ...
        votes_str = (
            ...
            f"SMC:{scores.get('smc', {}).get('veto', 0):+d}({scores.get('smc', {}).get('detail', 'N/A')})"
            f"PREM:{scores.get('_prem_disc', {}).get('score', 0):+d}"
        )
        ...
        votes = {
            ...
            'smc': {'score': scores.get('smc', {}).get('score', 50),
                    'veto': scores.get('smc', {}).get('veto', 0),
                    'detail': scores.get('smc', {}).get('detail', 'N/A')},
            'level': {'score': scores.get('_level_penalty', {}).get('score', 0),
                      'detail': f'{current_price/high_24h*100:.1f}% от 24h хая' if high_24h else 'N/A'},
            'prem_disc': {'score': scores.get('_prem_disc', {}).get('score', 0),
                         'detail': scores.get('_prem_disc', {}).get('detail', 'fair')},
        }

        # 🧠 Сохраняем голоса для VoteTracker (для учёта точности после закрытия сделки)
        # Решение принимается по base_score (без бонусов), но финальный score с бонусами сохраняется
        if passes_threshold:
            try:
        ...
        # 🧪 Фоновая валидация модулей (без сделки, по движению цены)
        try:
            get_vote_tracker().record_signal_prediction(symbol, votes, current_price)
        except Exception as _ve:
            logger.debug(f"[DE] VoteTracker prediction save error: {_ve}")
        ...
        try:
            ...
            log_signal(
                symbol=symbol,
                decision='buy' if passes_threshold else 'hold',
        ...
        if passes_threshold:
            return {
                ...
                'final_score': final_score,  # с бонусами — для сортировки между кандидатами
                'base_score': base_score,    # без бонусов — для контроля порога
                'threshold': threshold,
                'price': current_price,
            ...
            return {
                'approved': False,
                'reason': f"⏭️ Base={base_score:.0f} < {threshold}({strong_votes}≥75) {votes_str} bonus=p:{price_bonus:.0f}+r:{reversal_bonus:.0f}+b:{btc_bonus:+.0f} final={final_score:.0f}",
    ...
    def decide_short(self, symbol: str, confidence: float, trend: str,
        ...
        if current_positions_count >= max_short_positions:
            _short_limit_blocked = True
        else:
            _short_limit_blocked = False
        ...
        # ═══ БЛОКИРОВКА ВХОДА ПО ЛИМИТУ (после голосования) ═══════════
        if _short_limit_blocked and _result.action == 'enter':
            reason = f"Максимум {max_short_positions} шорт-позиций"
            self._save_veto_vote(symbol, current_price, reason, side='short')
            return Decision(symbol, 'hold', side='short', reason=reason, metadata=_result.metadata)
        
        return _result

    def determine_direction(self, symbol: str, confidence: float, trend: str,
                              rsi: float, current_price: float,
                              candles_5m: Optional[list] = None,
                              candles_1h: Optional[list] = None,
                              candles_4h: Optional[list] = None,
                              ...
                              btc_state: Optional[Dict] = None) -> Decision:
        """
        🆕 Единый проход ансамбля → LONG vs SHORT → арбитраж.
        
        Заменяет два независимых вызова decide_entry() + decide_short().
        Выполняет ОДИН набор вычислений для обоих направлений (кэшируется на 10 сек).
        BTC Direction Predictor — арбитр (+/- смещение).
        """
        # ─── Кэш: если символ уже проанализирован <10 сек назад ─────────
        now = time.time()
        cached = self._dir_cache.get(symbol)
        if cached and (now - cached['time']) < self._dir_cache_ttl:
            return Decision(symbol, cached['action'], side=cached['side'],
                            score=cached['score'], reason=cached['reason'])

        # ─── Нужна lazy-инициализация BTC Predictor ──────────────────────
        if self._btc_predictor is None:
            try:
                from btc_direction import BTC_Direction
                self._btc_predictor = BTC_Direction()
                logger.info(f"[DE→DIR] BTC Predictor загружен")
            except Exception as e:
                logger.warning(f"[DE→DIR] BTC Predictor не загружен: {e}")

        # ─── ДВА прохода ансамбля (LONG и SHORT) ─────────────────────────
        # Но с кэшем модулей: _evaluate_entry_ensemble кэширует VSA/Liq/CVD/etc
        long_result = self._evaluate_entry_ensemble(
            symbol, confidence, trend, rsi, current_price,
            candles_5m=candles_5m, candles_1h=candles_1h, candles_4h=candles_4h,
            side='long',
            cvd_data=cvd_data,
            high_24h=high_24h, low_24h=low_24h,
            btc_state=btc_state
        )
        short_result = self._evaluate_entry_ensemble(
            symbol, confidence, trend, rsi, current_price,
            candles_5m=candles_5m, candles_1h=candles_1h, candles_4h=candles_4h,
            side='short',
            cvd_data=cvd_data,
        ...
        # ─── Извлекаем LONG/SHORT scores ─────────────────────────────────
        long_score = min(100, max(0, int(long_result.get('final_score', 50))))
        short_score = min(100, max(0, int(short_result.get('final_score', 50))))
        logger.info(f"[DIR→SCORES] {symbol}: long={long_score} short={short_score}")

        # ─── BTC арбитр (смещение) ────────────────────────────────────────
        btc_bias = 0
        if self._btc_predictor:
            try:
                btc_dir = self._btc_predictor.calculate_bonus(0)
                # Ограничиваем бонус разумным диапазоном (-10..+10)
                btc_bias = max(-10, min(10, int(btc_dir)))
            except:
                pass
        # Дополнительно: BTC regime-флаг
        if btc_state:
            btc_trend = btc_state.get('trend', 'neutral')
            btc_regime = btc_state.get('regime', 'unknown')
            if btc_trend in ('down', 'falling') or btc_regime in ('dump', 'distribution', 'bearish_side'):
                btc_bias -= 3  # лёгкий сдвиг к SHORT
            if btc_trend in ('up', 'rising') or btc_regime in ('accumulation', 'recovery', 'pump'):
                btc_bias += 3  # лёгкий сдвиг к LONG

        # ─── Сравниваем ───────────────────────────────────────────────────
        delta = long_score - short_score + btc_bias
        ...
        if delta > 10:
            result = {
                ...
                'score': long_score,
                'reason': f'LONG>{short_score} (+{btc_bias}bias)',
            }
        elif delta < -10:
            result = {
                'action': 'enter', 'side': 'short',
                'score': short_score,
                'reason': f'SHORT>{long_score} ({btc_bias}bias)',
            }
        else:
            result = {
                'action': 'hold', 'side': 'neutral',
                'score': max(long_score, short_score),
                'reason': f'Нет консенсуса: LONG={long_score} SHORT={short_score} Δ={delta}',
            }

        # ─── Кэшируем ─────────────────────────────────────────────────────
        self._dir_cache[symbol] = {
            'time': now,
        ...
        # ─── Логирование в DE→/SHORT→ формат для vote_parser ───────────────
        _action_str = 'BUY' if result['action'] == 'enter' else 'HOLD'
        _side_tag = 'DE' if result['side'] == 'long' else 'SHORT' if result['side'] == 'short' else 'NEU'
        logger.info(f"[{_side_tag}→{_action_str}] {symbol}: side={result['side']} score={result['score']}/100 | {result['reason']}")
